...
int	ft_is_sorted_higher(t_lst *b_list)
{
	t_lst *iter;
...
t_lst	*ft_sort(t_lst *a_list)
{
	...
	while ((ft_is_sorted_higher(b_list)))
	{
		if (ft_closer_to_start_or_end(ft_find_lowest(b_list), b_list, ft_lst_size(b_list)))
			do_rrb(&b_list);
		else
			do_rb(&b_list);
	}
	while(b_list != NULL)
			do_pa(&a_list, &b_list);
	// liste_yazdir(a_list, "liste sonu");
	return(0);
}
